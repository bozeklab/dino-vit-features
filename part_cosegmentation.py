...
def find_part_cosegmentation(image_paths: List[str], elbow: float = 0.975, load_size: int = 224, layer: int = 11,
    ...
    if ckpt_path is not None:
        ...
        load_from_checkpoint(ckp_path=ckpt_path, student=extractor.model)
        print('Loading pre-trained checkpoint for saliency maps')
        load_from_checkpoint(ckp_path=ckpt_path, student=saliency_extractor.model)
    ...
    for img, img_path, num_patches, load_size, descs in zip(image_pil_list, image_paths, num_patches_list, load_size_list, descriptors_list):
        ...
        if save_dir is not None:
            ...
            fig, ax = plt.subplots()
            ax.axis('off')
            ax.imshow(parts.reshape(num_patches), vmin=0, vmax=num_labels - 1, cmap=cmap)
